...
class DmallFavoriteView(View):
    ...
    def get(self, request, object_id, *args, **kwargs):
        ...
        content_type = ContentType.objects.get_for_model(
            ProductSPU)  # 获取收藏对象的所属模型
        if request.user.is_authenticated:
            data = DmallFavorite.objects.filter(
                owner=request.user, content_type=content_type, object_id=object_id, is_del=False)  # 查询数据是否存在
            if data.exists():
                text = "已收藏"
            else:
                text = "收藏"
...
class DmallFavoriteCreateView(HasLoginRequired, BaseView, CreateView):
    ...
    def form_valid(self, form):
        ...
        form.instance.content_type = ContentType.objects.get_for_model(
            ProductSPU)  # 获取收藏对象的所属模型
        data = DmallFavorite.objects.filter(
            owner=self.request.user, content_type=form.instance.content_type, object_id=object_id)  # 查询数据是否存在
        if data.exists():
...
class DmallSearchView(BaseView, ListView):
    ...
    def get_queryset(self):
        # 重写queryset
        queryset = ProductSPU.objects.filter(
            is_del=False, is_show=True, is_release=True)
        keyword = self.request.GET.get('keyword')
        if not keyword:
            return queryset
        else:
            queryset = queryset.filter(Q(title__icontains=keyword) | Q(
                sub_title__icontains=keyword) | Q(desc__icontains=keyword))
            for product in queryset:
                # 设置默认显示多规格信息
                if product.spec_type == 1:
                    sku = product.skus.filter(
                        is_del=False, is_show=True).first()
...
class DmallAddressCreateView(HasLoginRequired, CreateView):
    ...
    fields = ('province', 'city', 'district', 'address',
              'signer_name', 'signer_mobile', 'email', 'is_default', )
    ...
    def form_valid(self, form):
        ...
        addr = DmallAddress.objects.filter(owner=self.request.user, is_default=True)
        if form.cleaned_data.get('is_default') and addr:
...
class DmallAddressHasDefault(DmallAddressUpdateView):
    ...
    def form_valid(self, form):
        ...
        DmallAddress.objects.filter(
            owner=self.request.user, is_default=True).update(is_default=False)
        return super().form_valid(form)
