...
from happy_shop.models import HappyShopOrderInfo, HappyShopAddress, HappyShopSKU
from django.core.cache import cache
from .views import BaseView


class HappyShopingCartView(LoginRequiredMixin, BaseView, TemplateView):
    """ 购物车页面 """
...
class HappyShopUserProfileView(LoginRequiredMixin, BaseView, TemplateView):
    """ 用户中心 """
    login_url = 'happy_shop:login'
    redirect_field_name = 'redirect_to'
...
class HappyShopUserOrdersView(HappyShopUserProfileView):
    """ 我的订单 """
    ...
    def get_order_status(self, status):
        return self.get_orders_queryset().filter(pay_status=status)
